...
private:
    ...
    seastar::future<> init_server();
    void client_round();
    const seastar::socket_address server_addr;
...
seastar::future<> ud_server_client::init_server() {
    return seastar::do_with(seastar::listen(server_addr), [this](server_socket& lstn) mutable {
        ...
        return do_until([this](){return rounds_left<=0;}, [&lstn,this]() {
            return lstn.accept().then([this](seastar::accept_result from_accept) {
                ...
                if (client_path) {
                    seastar::socket_address tmmp{unix_domain_addr{*client_path}};
                    BOOST_REQUIRE_EQUAL(cn_addr, seastar::socket_address{unix_domain_addr{*client_path}});
                }

                return seastar::do_with(cn.input(), cn.output(), [](auto& inp, auto& out) {
...
void ud_server_client::client_round() {
    auto cc = client_path ?
        seastar::engine().net().connect(server_addr, seastar::socket_address{unix_domain_addr{*client_path}}).get() :
        seastar::engine().net().connect(server_addr).get();
...
seastar::future<> ud_server_client::run() {
    return async([this] {
...
SEASTAR_TEST_CASE(unixdomain_server) {
    ...
    return seastar::do_with(std::move(uds), [](auto& uds){
        return uds.run();
    });
...
SEASTAR_TEST_CASE(unixdomain_text) {
    seastar::socket_address addr1{unix_domain_addr{"abc"}};
    BOOST_REQUIRE_EQUAL(format("{}", addr1), "abc");
    seastar::socket_address addr2{unix_domain_addr{""}};
    BOOST_REQUIRE_EQUAL(format("{}", addr2), "{unnamed}");
    seastar::socket_address addr3{unix_domain_addr{std::string("\0abc", 5)}};
...
SEASTAR_TEST_CASE(unixdomain_abort) {
    ...
    return seastar::do_with(std::move(uds), [sockname](auto& uds){
...
static seastar::socket_address autobind() {
...
static string to_string(seastar::net::packet p) {
    p.linearize();
...
SEASTAR_THREAD_TEST_CASE(unixdomain_datagram_autobind) {
    ...
    seastar::net::datagram dgram = chan2.receive().get();
...
SEASTAR_THREAD_TEST_CASE(unixdomain_datagram_named_bound) {
    ...
    auto named_receiver = make_bound_datagram_channel(seastar::socket_address{unix_domain_addr{socket_path}});
    ...
    sender.send(seastar::socket_address{unix_domain_addr{socket_path}}, "hihi").get();

    seastar::net::udp_datagram dgram = named_receiver.receive().get();
    string received = to_string(std::move(dgram.get_data()));
