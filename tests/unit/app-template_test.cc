...
#define BOOST_TEST_MODULE seastar::app_template
...
BOOST_AUTO_TEST_CASE(app_standard_memory_allocator) {
    ...
    // for testing seastar::app_template and underlying reactor's handling of different
    // settings
    ...
    int actual_status = app.run(
        ...
        [expected_status] {
            ...
            return seastar::sleep(2s).then([expected_status] {
                return seastar::make_ready_future<int>(expected_status);
            });
        });
